...
const filterStatus = computed(() => {
  ...
  if (searchQuery?.length > 0) {
    return taskStore.getTasks().filter((item) => item.name.toLowercase().includes(searchQuery))
  }
    ...
    <nav class="bg-white shadow" style="margin-top: 10px">
      <div class="mx-auto max-w-7xl px-2 flex items-center justify-between">
        <a href="#" class="flex items-center gap-4">
          <img src="/src/image/logo.png" alt="LOGO" class="w-[100px] h-[100px]" />
  ...
  <div class="flex justify-end mt-9 mx-20">
    ...
    <button class="btn btn-circle btn-outline mr-2">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 14 14">
        <path fill="#9FC3E9" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
    ...
    <label class="mr-8 input input-bordered- flex items-center gap-2 text-black">
      <input input type="text" v-model="searchQuery" class="grow" placeholder="Search" />
    </label> {{ searchQuery }}
      ...
      <div class="min-w-full">
        <table class="table-auto" style="table-layout: fixed">
          ...
          <thead>
            <tr class="bg-base-200 mt-4 md:mt-0">
              ...
              <th class="px-4 py-2 text-center md:text-left text-md font-semibold text-gray-700" style="
                  ...
                  color: #fff;
                ">
                <button style="display: flex; align-items: center" @click="sortByStatus(), toggleIcon()">
          ...
          <tbody>
            ...
            <tr class="itbkk-item" v-for="(item, index) in filterStatus" :key="item.id">
              ...
              <td class="itbkk-title px-4 py-2 text-center md:text-left text-sm text-gray-700">
                <label for="my_modal_6" @click="selectTodo(item.id)" style="display: block; width: 100%; height: 100%">
                  {{ item.title }}
                </label>
              </td>
                      ...
                      <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                        ...
                        <li>
                          <a style="width: 150px; margin-left: 17px" class="itbkk-button-delete btn"
                            @click="openModalToDelete(item.id, index)">Delete</a>
                        </li>
                      </ul>
                    </div>
                  </div>
